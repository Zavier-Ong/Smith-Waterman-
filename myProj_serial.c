...
void reverse(char *s);
SEQ* calc_2d_table(PERF* performance,int q_len,int d_len,char* q,char * d,int match,int mismatch,int gap);
...
int main(int argc, char *argv[]) {
  ...
  for(i=0;i<pairs;i++){
	...
	readPair(params,max_size_q,min_size_q,size_d);
  	result = calc_2d_table(performance,params->q_cnt,params->d_cnt,params->q,params->d,match,mismatch,gap);
